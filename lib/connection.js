...
module.exports = function (host) {
	 ...
	 * @param {Function} callback
	 */
	_self.bulkInsert = function (tableName, dataArrayToInsert, callback) {
		if (tableName && dataArrayToInsert) {
			if(dataArrayToInsert instanceof Array && dataArrayToInsert.length>0){
				let dataFirstRow = dataArrayToInsert[0];
				if (Object.size(dataFirstRow) > 0) {
					var mysqlQuery = 'INSERT INTO `' + tableName + '` (';
					var i = 0;
					for (var index in dataFirstRow) {
						mysqlQuery += index;
						mysqlQuery += (i + 1 == Object.size(dataFirstRow) ? ') ' : ', ');
						i++;
					}
				}
				mysqlQuery += 'VALUES ';
				...
				for(var i=0;i< dataArrayToInsert.length;i++){
					var data=dataArrayToInsert[i];
					mysqlQuery += comma+'(';
					var innerComma="";
					for (index in data) {
						mysqlQuery += innerComma+"'"+data[index]+"'";
						innerComma=",";
					}
					mysqlQuery +=') ';
					comma=",";
				...
				callback(new Error('mysql-json [bulkInsert]: data should be Array'), null);
			}
			
		}
		else {
			callback(new Error('mysql-json [bulkInsert]: Require at least a tableName and a data'), null);
		}
	};
